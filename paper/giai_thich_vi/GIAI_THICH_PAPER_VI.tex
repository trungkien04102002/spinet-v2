...
\textbf{(a) Deep learning cho phân độ đĩa đệm thắt lưng.} Jamaludin et al.\ giới thiệu SpineNet (CNN 2D kiểu VGG, GENODISC). Windsor et al.\ thay bằng 3D ResNet-34 đọc khối $(9,112,224)$ mỗi đĩa và đoán nhiều grade cùng lúc -- ta dùng đúng backbone + quy ước input này để nạp weights pretrained. Hallinan et al.\ đạt mức đồng thuận gần bằng bác sĩ cho hẹp ống sống/lỗ liên hợp; Nigru et al.\ kiểm định ngoại SpineNetV2 đa trung tâm: F1 lỗ liên hợp $\approx$0.84, balanced accuracy $\approx$0.69--0.70 (ta coi là ``trần'' của ảnh chỉ-đứng-dọc). Không hệ nào trong số đó \emph{tối ưu} cho bài 3-lớp mất cân bằng nặng (Nặng $<$5\%) -- chúng grade được, nhưng không xử riêng lớp hiếm. \emph{Chú ý} là cách vá quen thuộc: CBAM (Woo et al.) chỉnh trọng số kênh--không-gian, dùng rộng cho ảnh 2D; Lin et al.\ ghép nó với self-attention cho hẹp ống sống (F1 0.945 trên bộ của họ) -- ta mở rộng chú ý kênh--không-gian sang 3D đầy đủ. \emph{So với RSNA 2024 trước}: không so trực tiếp được vì khác cách đặt bài -- các lời giải công bố hoặc gộp segmentation+classification, hoặc dùng ensemble log-loss, hoặc là pipeline mở, không tách phân độ 3-lớp per-disc như ta. Ta cố tình giữ setting khó hơn (3-lớp đầy đủ, phân bố gốc, 1 mô hình, metric macro), mốc tương đương nhất là baseline 3-lớp công khai của Ning Shen (repo \texttt{nshen7}).

\textbf{(b) Mô hình ảnh--ngôn ngữ y khoa.} BiomedCLIP tiền-huấn-luyện kiểu CLIP trên PMC-15M (15 triệu cặp ảnh--chú-thích PubMed). Benchmark của Woerner \& Baumgartner cho thấy linear-probe BiomedCLIP vượt huấn luyện có giám sát khi ít nhãn -- đúng chế độ của lớp Nặng, nên ta dùng nó \emph{đông cứng} thay vì fine-tune. Kiến trúc gần ta nhất là RadCLIP (adapter gộp-lát cho ảnh khối); ứng dụng gần nhất là một mô hình kiểu CLIP huấn luyện từ đầu trên MRI thắt lưng ghép cặp để chẩn đoán đau lưng dưới (LumbarCLIP). Khác cả hai: ta giữ BiomedCLIP đông cứng và đọc embedding căn-chữ qua so-cosine, nên cùng mô hình đã train trả lời được hệ nhãn mới zero-shot chỉ bằng đổi prompt. Mô hình vision-language 3D MRI thuần vẫn đang là hướng mới nổi, để dành cho backbone tương lai.
\end{dichbox}
