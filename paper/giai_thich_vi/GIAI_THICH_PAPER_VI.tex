...
\textbf{Tiêu đề:} \emph{``A Frozen BiomedCLIP Branch for Imbalanced Lumbar Disc Grading and Zero-Shot Label Extension''} (bản trước có cụm ``as a Cross-Dataset Regularizer'' -- đã bỏ theo góp ý reviewer vì n=3 + parity, claim regularizer chỉ giữ ở phần Discussion)
...
\item \textbf{Phát hiện về điều hòa xuyên-dữ-liệu}: khác các nghiên cứu CBAM một-dữ-liệu trước, ta chỉ ra CBAM giúp chỉ số Nặng in-domain nhưng \emph{làm tụt} transfer SPIDER xuống dưới SpineNetV2 thuần, và nhánh BiomedCLIP đông cứng phục hồi lại ($+0.034$ so CBAM-only, $p=0.011$) $\Rightarrow$ phù hợp với việc nhánh mô hình nền điều hòa hiện tượng học-vẹt do attention.
\item \textbf{Kiến trúc lai hai nhánh}: CBAM-3D ResNet-34 + BiomedCLIP đông cứng + MLP-trộn, giải quyết điểm yếu của mô hình một-backbone vốn phải \emph{đánh đổi} nhạy lớp hiếm in-domain với robust xuyên-trung-tâm; ablation cho thấy hai nhánh bổ trợ chứ không thừa.
...
\begin{dichbox}{Bản dịch ý chính §2 (2 nhóm)}
\textbf{(a) Deep learning cho phân độ đĩa đệm thắt lưng.} Jamaludin et al.\ giới thiệu SpineNet (CNN 2D kiểu VGG, GENODISC). Windsor et al.\ thay bằng 3D ResNet-34 đọc khối $(9,112,224)$ mỗi đĩa và đoán nhiều grade cùng lúc -- ta dùng đúng backbone + quy ước input này để nạp weights pretrained. Nigru et al.\ kiểm định ngoại SpineNetV2 đa trung tâm: F1 lỗ liên hợp $\approx$0.84 (nhị phân), balanced accuracy $\approx$0.69--0.70 (ta coi là ``trần'' của ảnh chỉ-đứng-dọc). Không hệ nào trong số đó \emph{tối ưu} cho bài 3-lớp mất cân bằng nặng (Nặng $<$5\%) -- chúng grade được, nhưng không xử riêng lớp hiếm. \emph{Chú ý} là cách vá quen thuộc: CBAM (Woo et al.) chỉnh trọng số kênh--không-gian, dùng rộng cho ảnh 2D; Lin et al.\ ghép nó với self-attention cho hẹp ống sống (F1 0.945 trên bộ của họ) -- ta mở rộng chú ý kênh--không-gian sang 3D đầy đủ. \emph{So với RSNA 2024 trước}: không so trực tiếp được vì khác task -- Liu et al.\ báo F1 0.957 bằng MobileNetV2-UNet gộp seg+cls (5-fold), không tách phân độ 3-lớp per-disc; bài Kaggle chỉ báo weighted log-loss trên ensemble lớn. Ta cố tình giữ setting khó hơn (3-lớp đầy đủ, phân bố gốc, 1 mô hình, metric macro), mốc tương đương nhất là baseline 3-lớp công khai của Ning Shen (repo \texttt{nshen7}).

\textbf{(b) Mô hình ảnh--ngôn ngữ y khoa.} BiomedCLIP tiền-huấn-luyện kiểu CLIP trên PMC-15M (15 triệu cặp ảnh--chú-thích PubMed). Ảnh X-quang là thiểu số trong corpus nhưng vẫn vượt BioViL trên RSNA Pneumonia; benchmark của Woerner \& Baumgartner cho thấy linear-probe BiomedCLIP vượt huấn luyện có giám sát khi ít nhãn -- đúng chế độ của lớp Nặng, nên ta dùng nó \emph{đông cứng} thay vì fine-tune. Kiến trúc gần ta nhất là RadCLIP (thêm adapter gộp-lát cho ảnh khối). Khác biệt ở \emph{mục đích}: ta giữ BiomedCLIP đông cứng và đọc embedding căn-chữ qua so-cosine, nên cùng mô hình đã train trả lời được hệ nhãn mới zero-shot chỉ bằng đổi prompt. Mô hình vision-language 3D MRI thuần chưa public lúc làm.
\end{dichbox}
