...
\item \textbf{Phát hiện về điều hòa xuyên-dữ-liệu}: chú ý kênh--không-gian giúp chỉ số Nặng trên dataset huấn luyện (RSNA), nhưng \emph{lợi thế in-domain đó không chuyển giao} sang dataset mới (SPIDER); thêm nhánh đa phương thức đông cứng \emph{khôi phục độ bền xuyên-dữ-liệu} ($+0.034$ so CBAM-only, $p=0.011$) $\Rightarrow$ nhánh nền như một \emph{lá chắn} chống attention học-vẹt dataset huấn luyện (quan sát hành vi, không phải chứng minh cơ chế).
\item \textbf{Thiết kế lai hai nhánh}: ghép một bộ mã hóa \emph{chú ý thể tích} với một \emph{tiên nghiệm đa phương thức đông cứng} qua một đầu trộn nhỏ học được, giải quyết điểm yếu của mô hình một-backbone vốn phải \emph{đánh đổi} nhạy lớp hiếm in-domain với robust xuyên-trung-tâm; ablation cho thấy hai nhánh bổ trợ chứ không thừa (hiện thực: CBAM-3D ResNet-34 + BiomedCLIP).
