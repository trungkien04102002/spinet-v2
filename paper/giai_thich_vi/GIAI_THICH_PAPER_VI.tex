...
\large Hybrid CBAM-3D + BiomedCLIP cho phân độ thoái hóa đĩa đệm thắt lưng}
...
\textbf{Tiêu đề:} \emph{``A Frozen BiomedCLIP Branch as a Cross-Dataset Regularizer for Imbalanced Lumbar Disc Grading and Zero-Shot Label-Space Extension''}
\\ = ``Một nhánh BiomedCLIP đông cứng đóng vai bộ điều hòa xuyên-dữ-liệu cho bài phân độ đĩa đệm thắt lưng mất cân bằng và mở rộng không gian nhãn theo kiểu zero-shot''.

\begin{dichbox}{Bản dịch ý chính của Abstract}
Phân độ tự động bệnh lý đĩa đệm thắt lưng trên MRI rất hữu ích trên lâm sàng nhưng khó: các lớp mức độ \emph{mất cân bằng nặng} và \emph{thiếu dữ liệu khối có nhãn}. Chúng tôi đề xuất mô hình lai hai nhánh: trộn 3D ResNet-34 + CBAM với encoder ảnh--chữ BiomedCLIP \emph{đông cứng}, huấn luyện trên RSNA 2024. Nhánh CBAM-3D nắm bối cảnh giải phẫu xuyên-lát; nhánh BiomedCLIP cung cấp \emph{tiên nghiệm ngữ nghĩa} từ tiền-huấn-luyện y sinh quy mô lớn; một MLP-trộn nhỏ gộp hai luồng thành embedding căn-chỉnh-với-chữ, dùng cho cả phân loại có giám sát lẫn mở rộng nhãn zero-shot. Với focal loss, trọng số lớp căn-bậc-hai, oversampling lớp hiếm và augmentation kiểu SpineNetV2, qua 3 seed mô hình lai nâng Recall lớp Nặng từ 12.3\% lên 48.6\%, F1 Nặng 0.152$\to$0.356, AUC Nặng 0.872$\to$0.899. Không cần tinh chỉnh trên SPIDER, cùng mô hình đó mở rộng zero-shot sang 8 nhãn SPIDER chưa từng thấy qua prompt chữ (trung bình F1 = 0.362), với phần cải thiện tập trung ở các nhãn hình thái đĩa gần với hệ huấn luyện. Khi chuyển giao có giám sát qua 3 seed, mô hình lai \emph{ngang} SpineNetV2 về F1 tổng hợp (0.653 vs 0.646) và \emph{vượt} bản chỉ-CBAM (0.619) $+0.034$ ($t$-test bắt cặp, $p=0.011$); chúng tôi diễn giải đây là bằng chứng nhánh BiomedCLIP đông cứng cung cấp \emph{tín hiệu điều hòa} dưới chuyển giao xuyên-dữ-liệu.
\end{dichbox}
...
Chúng tôi giải quyết cả hai bằng một mô hình lai. Bốn đóng góp (theo trọng tâm là quan sát chuyển giao):
\begin{enumerate}[leftmargin=1.5em,itemsep=1pt]
\item \textbf{Quan sát thực nghiệm về điều hòa xuyên-dữ-liệu}: thêm CBAM làm tụt F1 chuyển giao $0.027$ (vượt biến thiên giữa các seed) $\Rightarrow$ CBAM giúp in-domain nhưng hại tổng quát hóa; mô hình lai phục hồi trên CBAM $+0.034$ ($p=0.011$).
\item \textbf{Kiến trúc lai hai nhánh}: CBAM-3D ResNet-34 + BiomedCLIP đông cứng + MLP-trộn; hai nhánh bổ trợ (in-domain vs robust xuyên-trung-tâm).
\item \textbf{Mở rộng không gian nhãn xuyên-dữ-liệu}: định tuyến embedding trộn qua so-cosine với prompt chữ $\Rightarrow$ chuyển sang 8 nhãn SPIDER không cần tinh chỉnh (F1 0.362); với tinh chỉnh nhẹ, dẫn đầu ở lớp hiếm.
\item \textbf{Pipeline xử lý mất cân bằng} (focal loss, trọng số căn, oversampling, augmentation) nâng Recall Nặng 12.3\%$\to$48.6\%.
\end{enumerate}
Theo hiểu biết của nhóm, chưa có công trình peer-review nào áp BiomedCLIP cho RSNA 2024, cũng chưa ai báo cáo mở rộng nhãn zero-shot trên cùng kiến trúc cho MRI cột sống thắt lưng.
\end{dichbox}
...
\begin{dichbox}{Bản dịch ý chính §2 (4 nhóm)}
\textbf{(a) Phân độ đĩa đệm thắt lưng.} Jamaludin et al.\ giới thiệu SpineNet (CNN 2D kiểu VGG). Windsor et al.\ nâng lên 3D ResNet-34 chạy trên khối $(9,112,224)$ -- đây là nền kiến trúc ta dùng. Nigru et al.\ kiểm định ngoại SpineNetV2 đa trung tâm, báo F1 lỗ liên hợp $\approx$0.84, balanced accuracy $\approx$0.69--0.70 (ta coi là ``trần'' của ảnh chỉ-đứng-dọc).

\textbf{(b) Chú ý cho ảnh y khoa.} CBAM (Woo et al.) được dùng rộng cho bài 2D. Lin et al.\ kết hợp chú ý kênh--không-gian với multi-head self-attention để phân loại \emph{hẹp ống sống thắt lưng} (F1 0.945 trên bộ của họ). Ta mở rộng chú ý kênh--không-gian sang 3D đầy đủ và ghép với nhánh mô hình nền để xử lý phân độ 3-lớp đa-mức dưới mất cân bằng.

\textbf{(c) Mô hình ảnh--ngôn ngữ y khoa.} BiomedCLIP tiền-huấn-luyện kiểu CLIP trên PMC-15M (15 triệu cặp ảnh--chú-thích PubMed). Ảnh X-quang chỉ là thiểu số trong corpus nhưng BiomedCLIP vẫn vượt BioViL trên RSNA Pneumonia. Benchmark gần đây cho thấy linear-probe BiomedCLIP vượt huấn luyện có giám sát ở chế độ few-shot $\Rightarrow$ động lực dùng nó làm bộ trích đặc trưng đông cứng cho bộ dữ liệu nhỏ. RadCLIP mở rộng CLIP với adapter gộp-lát cho ảnh khối -- gần kiến trúc của ta nhất. Mô hình vision-language 3D MRI thuần chưa public lúc làm.

\textbf{(d) Định vị so với các phương pháp RSNA 2024 trước.} Các kết quả RSNA 2024 đã công bố không so trực tiếp được vì thiết lập khác: Liu et al.\ báo F1 0.957 bằng MobileNetV2-UNet gộp phân-đoạn+phân-loại, đánh giá 5-fold (điểm gộp, không tách phân độ 3-lớp). Các bài Kaggle chỉ báo weighted log-loss trên ensemble 10--20 backbone (không quy ra F1). Thiết lập của ta cố tình khó hơn: phân độ 3-lớp đầy đủ, cả 5 tầng, phân bố mất cân bằng gốc, đánh giá một mô hình đơn với metric macro (Mean F1 0.527, Severe F1 0.356). Mốc tham chiếu tương đương nhất là baseline 3-lớp công khai của nshen7.
\end{dichbox}
...
\subsection{4.5 Trực quan hóa \& Thảo luận}
...
%==========================================================
\section{Mục 5 -- Summary (Kết luận)}
\begin{dichbox}{Bản dịch ý chính §5}
Chúng tôi trình bày kiến trúc lai CBAM-3D + BiomedCLIP đông cứng cho phân độ đĩa đệm thắt lưng RSNA 2024. Nó cải thiện các chỉ số lớp Nặng (3 seed: Recall $\sim$4$\times$, F1 2.3$\times$, AUC 0.899) ở mức đánh đổi accuracy có kiểm soát, và cho phép mở rộng nhãn zero-shot sang 8 nhãn SPIDER chưa thấy qua prompt chữ. Nghiên cứu 3 seed trên SPIDER cho thấy CBAM giúp in-domain nhưng làm tụt F1 cross-dataset dưới SpineNetV2, còn nhánh BiomedCLIP đông cứng phục hồi mất mát đó -- hiệu ứng chúng tôi diễn giải là điều hòa xuyên-dữ-liệu. Hướng tương lai: phân tích biểu diễn (cơ chế), adapter gộp-lát kiểu RadCLIP, và hợp nhất đa-mặt-phẳng với ảnh axial.
\end{dichbox}
...
\item BiomedCLIP đông cứng \& 2D; mô hình 3D VLM thật chưa public.
