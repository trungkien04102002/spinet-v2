...
backbone + train heads, 15 epochs) cho thấy Hybrid vượt 3 baseline trên
mean F1 ($0.660$ vs Baseline $0.649$, CBAM $0.621$, BMC-only $0.617$)
và Spondylolisthesis Severe Recall $0.40$ vs Baseline $0.10$, CBAM
$0.00$ --- bằng chứng cụ thể của text-guided minority recovery. Đây là
pipeline đầu tiên áp dụng BiomedCLIP cho RSNA 2024 lumbar grading mà
chúng tôi biết. \emph{Single-seed (seed=42) cho cả hai phase}.
\end{abstract}
...
\item \textbf{Cross-dataset transfer learning trên SPIDER 8 nhãn}: với
chỉ 15 epoch fine-tune heads, Hybrid đạt mean F1 $=0.660$ (vs Baseline
$0.649$, CBAM $0.621$, BMC-only $0.617$); win 4/8 conditions trên F1
và 4/8 trên AUPRC. Trong đó Spondylolisthesis (minority $3\%$): Recall
$0.40$ vs Baseline $0.10$, CBAM/BMC-only $0.00$ --- evidence cho
text-guided minority class recovery.
\item \textbf{CBAM-hurts-cross-dataset finding}: CBAM (over-fit RSNA)
\emph{thua} Baseline trên SPIDER ($0.621$ vs $0.649$ mean F1), nhưng
Hybrid (CBAM $+$ BiomedCLIP) bù lại được. Bằng chứng BiomedCLIP knowledge
cải thiện cross-dataset robustness.
\end{itemize}
...
\subsection{Cross-dataset SPIDER 8-label transfer learning}

\noindent\textbf{Setup}: 4 cấu hình (Baseline / CBAM / BMC-only /
Hybrid) đều khởi tạo từ RSNA checkpoint tương ứng, freeze backbone,
train 15 epoch trên SPIDER training split (940/234 IVD, seed=42, batch=16,
lr=$10^{-3}$, weighted CE loss, RTX 4090). Đánh giá trên 234 IVD val
qua 8 nhãn $\times$ 5 metric (Acc, F1, AUC, AUPRC, per-class Severe Recall).

\subsubsection*{Bảng aggregate (mean across 8 conditions)}

\begin{table}[h]
\centering
\caption{SPIDER 8-label transfer learning --- 4 configs $\times$ 4
metric chính. \best{Bold} = best per row. Single seed.}
...
Train time (min)      & 19.4       & 19.9   & \best{13.9} & 18.5 \\
Inference (ms/sample) & 62.0       & 62.4   & \best{60.0} & 61.6 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Findings aggregate}:
\begin{itemize}[leftmargin=*]
\item Hybrid win 3 primary metrics (F1 / AUC / AUPRC). Baseline win
Accuracy (predict majority class).
\item Hybrid vs CBAM $= +0.039$ F1 ($+6.3\%$) --- gap có ý nghĩa: CBAM
over-fit RSNA features bị hurt khi transfer.
\item Hybrid vs Baseline $= +0.011$ F1 ($+1.7\%$) --- gap nhỏ, nằm
trong noise của single-seed. \emph{Cần multi-seed để khẳng định}.
\item BMC-only train nhanh nhất (13.9 min) nhưng thấp nhất trên F1
($0.617$) --- text-only branch không đủ, cần CBAM features.
\end{itemize}

\subsubsection*{Per-condition F1 macro (chi tiết)}

\begin{table}[h]
\centering
\caption{F1 macro theo từng condition trên SPIDER (8 nhãn). Cột Winner
in \best{bold}.}
...
\midrule
\textbf{Wins per config} & 4 & 1 & 2 & 4 & --- \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Per-condition insight}: Hybrid và Baseline cùng win 4
conditions, nhưng Hybrid \emph{lead Spondylolisthesis bằng margin lớn}
($+0.111$ vs Baseline) --- đây là minority class hiếm nhất ($\sim
3\%$). CBAM kém nhất, chỉ win 1 condition (LOW Endplate, $+0.005$).

\subsubsection*{Severe / Minority class Recall (zero-shot benefit)}

\begin{table}[h]
\centering
\caption{Recall của \emph{minority class} (positive label) cho mỗi
binary condition + Pfirrmann Grade 4. Chỉ số quan trọng lâm sàng.}
\label{tab:spider_minority_recall}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
\rowcolor{tablehdr}
Condition (minority class) & Prev. & Base & CBAM & BMC-only & Hybrid \\
\midrule
Pfirrmann Grade 4 (severe)         & 19\% & 0.474 & 0.447 & 0.342 & \best{0.579} \\
Modic Type 1 (rare)                & $<$5\%& 0.000 & 0.000 & 0.000 & 0.000 \\
Modic Type 3 (rare)                & $<$1\%& 0.000 & 0.000 & 0.000 & 0.000 \\
Disc Narrowing (Yes)               & 38\% & 0.742 & 0.730 & 0.798 & \best{0.820} \\
\textbf{Spondylolisthesis (Yes)}   & 3\%  & 0.100 & 0.000 & 0.000 & \best{0.400} \\
...
\noindent\textbf{Bomb finding}: \textbf{Spondylolisthesis} (positive
chỉ $3\%$ samples): Hybrid recover $40\%$ minority, Baseline chỉ
$10\%$, \emph{CBAM \& BMC-only fail hoàn toàn ($0\%$)}. Đây là evidence
cụ thể text-guided fusion giúp minority class --- không phải artifact
của single config.
