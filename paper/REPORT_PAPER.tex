...
from $0.47$ to $0.53$. To our knowledge this is the first
BiomedCLIP-based pipeline reported on RSNA 2024 lumbar
classification.
\end{abstract}
...
Aktan~et~al.~\cite{aktan2025}            & '25 & RSNA    & \best{.957}\,/\,—   & Bin., bal., joint seg \\
...
appears higher than our $0.343$, but reflects a per-condition
evaluation (likely canal-only) on a different validation split; our
...
pooled CBAM--Baseline noise ($\sigma = 0.010$). This is a substantive
...
seed standard deviation ($\sigma = 0.010$). CBAM appears to over-fit
...
interpreted as deployment-ready. Closing the remaining recall gap
likely requires additional sources of evidence: axial T2 reformats
for foraminal stenosis, larger training corpora with synthetic
augmentation for the Severe class, and human-in-the-loop refinement
on edge cases identified by the model.
