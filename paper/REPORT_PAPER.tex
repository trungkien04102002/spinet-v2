...
\author{\IEEEauthorblockN{Trung Kien Ha\IEEEauthorrefmark{1},
        ...
        \IEEEauthorblockA{\IEEEauthorrefmark{1}Faculty of Computer Science and Engineering,
                          Ho Chi Minh City University of Technology (HCMUT),
                          VNU-HCM,
                          Ho Chi Minh City, Vietnam.\\
                          Email: kien.ha@hcmut.edu.vn,\,
                          nhanphan@hcmut.edu.vn}}
...
We address both problems with a single hybrid model. Our four
contributions, ordered by strength of multi-seed evidence, are:
\begin{itemize}
\item A \textbf{cross-dataset regularization finding}: in a 3-seed
      SPIDER transfer experiment, adding CBAM to the ResNet-34
      backbone degrades aggregate F1 by $-0.027$ (exceeds pooled
      seed $\sigma$), revealing that the same attention module which
      helps in-domain RSNA Severe metrics hurts cross-center
      generalization. The proposed Hybrid (CBAM~+~frozen BiomedCLIP)
      recovers above the CBAM-only configuration by $+0.034$
      ($> 2\sigma$, significant), establishing the BiomedCLIP branch
      as a regularizer.
\item A \textbf{two-branch hybrid architecture} that combines a CBAM-3D
      ...
      fusion head. Ablation confirms both branches contribute on
      complementary axes (in-domain Severe metrics vs.\ cross-center
      robustness).
\item \textbf{Cross-dataset label-space extension}: by routing the fused
      ...
      mean F1~=~0.362 zero-shot; with lightweight supervised transfer
      (3 seeds), Hybrid leads on rare-class recall (Disc Herniation
      Yes $0.36~\pm~0.08$ vs.\ $0$ for Baseline/CBAM).
\item A \textbf{class-imbalance handling pipeline} (focal
      ...
      recall from 11.5\% to 46.4\% on RSNA 2024 (single-seed; 3-seed
      RSNA evaluation pending, see \S\ref{sec:disc}).
\end{itemize}
...
Table~\ref{tab:sota} summarizes published methods reporting on RSNA
2024 or closely related lumbar spine MRI grading tasks. Direct
numerical comparison is not possible across rows because each prior
method targets a different problem setup. Specifically: (i)
binarization of the original 3-class severity grading into
normal-vs-abnormal collapses the clinically critical Moderate--Severe
boundary~\cite{lin2024cbam,aktan2025}; (ii) single-level evaluation,
typically L4--L5 only, ignores spatial heterogeneity across the lumbar
spine~\cite{lin2024cbam}; (iii) balanced-subset construction
eliminates the very class imbalance that motivates the
problem~\cite{lin2024cbam,aktan2025,phaphuangwittayakul2026}; and (iv)
Kaggle competition entries report only weighted log-loss on ensembles
of 10--20 backbones with extensive test-time augmentation, which is
neither a single-model evaluation nor convertible to F1. Our setup is
intentionally the more difficult variant: full 3-class severity
grading across all five intervertebral disc levels on the natively
imbalanced distribution, evaluated as a single model. We report our
numbers alongside the published values in Table~\ref{tab:sota} with
explicit setup footnotes; absolute-number comparisons should be
interpreted under those caveats.
...
\midrule
Hallinan~et~al.~\cite{hallinan2021}      & '21 & Priv.   & $\kappa$ .89--.96 & Bin., diff.\,metric \\
Nigru~et~al.~\cite{nigru2024}            & '24 & Priv.   & .84\,/\,$\kappa$.46--.74 & Foram.\,binary \\
...
Kaggle Top-3\,$^\ddagger$ \cite{kaggle2024} & '24 & RSNA & log-loss only & Ensemble $\geq$ 10 \\
\midrule
...
ext.~=~external dataset. $^\ddagger$Kaggle log-loss does not convert
to F1; ensembles of 10--20 backbones are not single-model.\par}
\end{table}
...
Prior published work on the SPIDER dataset has focused predominantly
on segmentation~\cite{vandergraaf2024spider}; multi-task classification
benchmarks reporting per-condition F1 across the full 8-label schema
remain limited. To our knowledge, this is the first reported
evaluation of cross-dataset zero-shot transfer (RSNA $\to$ SPIDER)
using text-prompt-based label-space extension.
...
terms but consistent across nearly every metric. We use the term
\emph{complementary} in the empirical sense that each branch alone
falls short of the Hybrid; we do not claim a formal synergy effect,
which would require an interaction-term test (e.g.\ ANOVA across
the four ablation cells with multiple seeds) and is deferred to
future work.

\textbf{Why CBAM, given the cross-dataset cost?} A natural question
arises from the 3-seed SPIDER analysis: if CBAM hurts cross-dataset
F1 (Baseline 0.646 vs.\ CBAM 0.619, a regression of $-0.027$ that
exceeds pooled seed $\sigma$), why include CBAM at all? Two empirical
facts justify CBAM as a necessary component rather than a flaw.
First, on the in-domain RSNA task, CBAM is the single largest source
of Severe-class improvement: Severe F1 rises from $0.149$ (Baseline) to
$0.304$ (CBAM-only), a $+0.155$ absolute gain that the BiomedCLIP
branch alone cannot match (BMC-only Severe F1 $0.229$). The Hybrid
inherits this RSNA-side gain and adds a further $+0.039$ to reach
Severe F1 $0.343$. Second, the cross-dataset asymmetry is itself a
finding: rather than a design flaw, it documents the cost of strong
in-domain feature specialization. The Hybrid is therefore best
understood as combining CBAM's in-domain selectivity with BiomedCLIP's
distribution-invariant semantic prior --- not as repairing a CBAM
defect, but as deliberately combining components with complementary
strengths.

\textbf{CBAM helps in-domain but hurts cross-dataset transfer; BiomedCLIP
restores robustness.} On SPIDER (different
sites, scanners, label schema), CBAM Mean F1 ($0.619 \pm 0.010$)
\emph{drops below} the plain ResNet-34 Baseline ($0.646 \pm 0.002$),
a $\Delta = -0.027$ regression that exceeds the pooled CBAM--Baseline
seed standard deviation ($\sigma_{\text{pooled}} = 0.007$,
$|\Delta|/\sigma = 3.8$). CBAM appears to over-fit
RSNA-specific spatial patterns that do not transfer across centers.
Hybrid ($0.653 \pm 0.014$) recovers above CBAM by $+0.034$ ($> 2\sigma$,
significant) and matches Baseline within seed variability. We
therefore frame Hybrid not as an additive feature combination that
beats Baseline, but as a regularizer that \emph{restores}
Baseline-level performance when CBAM is added: the BiomedCLIP branch
provides a robust fallback when CBAM's in-domain bias is misaligned
with the target distribution. The same pattern explains why CBAM-only
fails on the SPIDER Spondylolisthesis minority class (Recall
$0.000 \pm 0.000$) while Hybrid recovers $0.300 \pm 0.098$ via
text-guided generalization, and why Disc Herniation positive recall
goes from $0$ for Baseline/CBAM to $0.356 \pm 0.079$ for Hybrid. A
trivial-ensemble alternative explanation --- that the regularization
effect reduces to logit averaging of Baseline and BMC-only branches
--- cannot be fully ruled out from the current experiments; a direct
comparison to a Baseline+BMC-only logit-average ensemble is deferred
to future work.

\textbf{Severe Recall remains below clinical deployment thresholds.}
The reported Severe Recall of $46.4\%$ on RSNA, while a 4.0$\times$
improvement over baseline, is below typical clinical screening
sensitivity thresholds, which generally require recall above $80\%$
to be acceptable as a triage tool. The findings here establish that
the methodology efficacy is real (consistent improvements across
metrics, conditions, and a second dataset) but should not yet be
interpreted as deployment-ready. At the current operating point the
model is best positioned as a \emph{second-reader assistance tool}
that flags candidate Severe cases for a radiologist to confirm,
rather than as a standalone screening system. Closing the remaining
recall gap likely requires additional sources of evidence: axial T2
reformats for foraminal stenosis, larger training corpora with
synthetic augmentation for the Severe class, and human-in-the-loop
refinement on edge cases identified by the model.
...
\textbf{Limitations.} (i) RSNA results are reported from a single
seed (42); the SPIDER transfer learning experiments use 3 seeds
($\{42, 123, 456\}$). A 3-seed RSNA sweep is pending and will be
included in the camera-ready version. We additionally commit to
reporting bootstrap 95\% confidence intervals on the RSNA Severe F1,
Severe Recall, and Severe AUPRC by resampling the validation set
($n = 1000$ resamples), which provides uncertainty quantification on
the headline numbers independently of the multi-seed effort.
(ii) BiomedCLIP is
...
unexplored ablations. (iv) RSNA 2024 ground-truth labels were
contributed by a multi-annotator, multi-center crowdsourced
labeling effort~\cite{kaggle2024}; per-class inter-rater agreement
statistics for the Severe class were not released alongside the
public dataset, so the upper bound on achievable Severe metrics
reflects label noise as well as model capacity.
