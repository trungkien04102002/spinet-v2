...
conditions. All metrics on val (234 IVDs). \textbf{Bold} = best per
row. Single seed (42).}
...
\midrule
Inference (ms/sa) & 62.0         & 62.4    & \best{60.0} & 61.6 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Aggregate result.} Hybrid wins three of the four primary
metrics (F1, AUC, AUPRC); Baseline wins Mean Accuracy by predicting
majority classes. The Hybrid vs.\ Baseline F1 gap is $+0.011$, small
in absolute terms and within the noise expected from a single seed
on a 234-sample validation split; we report multi-seed results in a
later revision once those runs complete. The Hybrid vs.\ CBAM gap is
$+0.039$ F1 ($+6.3\%$), which is more substantial and consistent
with the observation that CBAM (overfit to RSNA-specific spatial
patterns) generalizes less cleanly across centers than the
text-aligned Hybrid representation. We discuss this trade-off
explicitly in \S\ref{sec:disc}.
...
(positive prevalence $2.8\%$), Hybrid recovers $40.0\%$ of positive
cases vs.\ Baseline $10.0\%$ and CBAM/BMC-only both $0.0\%$;
Hybrid's Spondylolisthesis F1 is $0.676$ vs.\ Baseline $0.565$
($+0.111$). This is the same minority-class recovery pattern
observed on RSNA Severe (Recall $11.5\% \to 46.4\%$), suggesting
that text-guided fusion provides a consistent benefit on
underrepresented classes across datasets, even when the aggregate F1
gap is small. Modic Types~1 and 3 (population prevalence
$0.3\%$ and $0.5\%$ respectively) remain at $0\%$ recall across all
four configurations and are not amenable to 15-epoch frozen-backbone
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Condition (prevalence)              & Base   & CBAM   & BMC    & \best{Hybrid} \\
\midrule
Spondylolisthesis Yes (2.8\%)       & 0.100  & 0.000  & 0.000  & \best{0.400} \\
Disc Herniation Yes (4.7\%)         & 0.000  & 0.000  & \best{0.455} & 0.273 \\
Pfirrmann Grade~4 (19\%)            & 0.474  & 0.447  & 0.342  & \best{0.579} \\
...
restores robustness.} The SPIDER transfer learning results in
\S\ref{sec:spider_transfer} reveal an asymmetry. On RSNA, CBAM
improves Severe F1 (0.149 $\to$ 0.304) by sharpening attention on
small lesions in the same imaging distribution. On SPIDER (different
sites, scanners, label schema), CBAM mean F1 (0.621) actually
\emph{drops below} the plain ResNet-34 Baseline (0.649), a 4.3\%
relative degradation. CBAM appears to over-fit RSNA-specific spatial
patterns that do not transfer across centers. Hybrid (0.660) recovers
above Baseline by fusing CBAM with BiomedCLIP's pretraining-derived
semantic prior, which is dataset-agnostic by construction. This
positions the hybrid not merely as an additive feature combination,
but as a regularizer: the BiomedCLIP branch provides a robust
fallback when CBAM's in-domain bias is misaligned with the target
distribution. The same pattern explains why CBAM-only fails on the
SPIDER Spondylolisthesis minority class (Recall $0\%$) while Hybrid
recovers $40\%$ via text-guided generalization.
