...
\begin{itemize}
    \item \textbf{Thoái hóa đĩa đệm (Thang đo Pfirrmann grade 1-5):} đĩa đệm mất nước, giảm chiều cao, tín hiệu T2 giảm dần.
...
\begin{itemize}
    ...
    \item \textbf{Biến đổi modic (Modic changes type 1--3):} biến đổi tủy xương dưới sụn endplate, biểu hiện rõ trên T1 và T2.
\end{itemize}
...
Trong lĩnh vực chẩn đoán hình ảnh, dữ liệu được lưu trữ dưới nhiều định dạng khác nhau, mỗi định dạng có những đặc điểm riêng về cấu trúc và ứng dụng. Hai trong số các định dạng phổ biến nhất trong nghiên cứu và lâm sàng là DICOM và NIfTI.
...
\textbf{Đốt sống:} Trên cả CT và MRI, phần xương vỏ (cortical bone) của thân đốt sống có tín hiệu thấp (màu đen), trong khi phần tủy xương (bone marrow) bên trong có tín hiệu cao hơn. CT là phương tiện tốt nhất để đánh giá chi tiết cấu trúc xương.
...
trúc như \textbf{Hình 2.6}. Trong đó:
...
\begin{figure}[!htb]
        \centering
        \includegraphics[scale=0.6]{cslt/perceptron.png}
...
tuyến như \textit{Sigmoid, Tanh, ReLU, LeakyReLU} được minh họa ở \textbf{Hình 2.7}. 


\begin{figure}[!htb]
        \centering
        \includegraphics[scale=0.7]{cslt/act.png}
        \caption{Các hàm phi tuyến được sử dụng trong Perceptron}
        \label{fig:u2net3d-abstract}
    \end{figure}
...
Mạng tích chập có 02 phần chính: \textbf{Lớp trích lọc đặc trưng của ảnh} (Conv, Relu và Pool) và \textbf{Lớp phân loại} (FC và softmax).\\
\begin{figure}[!htb]
        \centering
        \includegraphics[scale=0.5]{cslt/cnn.png}
        \caption{Mô hình CNN cơ bản}
        \label{fig:u2net3d-abstract}
\end{figure}
...
\cite{ronneberger2015unet}. Tên gọi "U-Net" xuất phát từ hình dạng chữ U đặc trưng của kiến trúc này. Về bản chất, U-Net là một mô hình \textbf{Encoder-Decoder} được cải tiến, bao gồm hai phần chính: một \textbf{đường đi xuống (contracting path)} để nắm bắt ngữ cảnh và một \textbf{đường đi lên (expansive path)} để tái tạo ảnh và định vị chính xác.

\begin{figure}[!htb]
        \centering
        \includegraphics[scale=0.4]{cslt/unet-arch.jpeg}
        \caption{Kiến trúc U-Net}
        \label{fig:u2net3d-abstract}
\end{figure}
...
\section{Cơ chế anttention CBAM (Convolutional block attention module)}
