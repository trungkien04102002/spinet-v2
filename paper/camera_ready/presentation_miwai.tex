...
{\small Frozen prior anchors the scarce Severe class; attention zooms into the lesion.}
\end{block}
\column{.48\textwidth}
\centering
...
\end{equation*}
\begin{itemize}\small
    \item[--] \textbf{Source (RSNA)}, three prompts per condition, one per grade:
          \textit{``normal or mild / moderate / severe spinal canal stenosis''}.
    \item[--] \textbf{Target (SPIDER)}: \textit{``a magnetic resonance image of [label]''}.
    \item[--] Anchors $t_k$ are encoded \textbf{once} and reused; the volume is the
          only per-case input, and $\{t_k\}$ sits where a weight matrix would.
...
\begin{itemize}
    ...
    \item[--] Grad-CAM~\cite{selvaraju2017gradcam}: one Severe case, illustrative only.
\end{itemize}
\column{.5\textwidth}
\centering
...
\end{table}
\begin{itemize}\small
    \item[--] Hybrid $\approx$ SpineNetV2 (\textbf{parity}, $p{=}0.57$); SpineNetV2 leads on AUPRC.
