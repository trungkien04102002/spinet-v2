...
\end{itemize}
\vspace{4pt}
{\scriptsize Grading is manual today, and readers disagree~\cite{nigru2024}.}
\column{.58\textwidth}
\centering
...
\column{.52\textwidth}
\begin{itemize}\small
    \item[\textbf{1.}] \textbf{Focal loss}~\cite{lin2017focal} ($\gamma{=}2$): focus on hard cases.
    \item[\textbf{2.}] \textbf{$\sqrt{}$ class weights}: gradient ratio $15\times \to 4\times$.
    \item[\textbf{3.}] \textbf{Minority oversampling}~\cite{buda2018imbalance} (3 to 5$\times$): see Severe more often.
    \item[\textbf{4.}] \textbf{Augmentation}~\cite{shorten2019augmentation}: flips, $\pm10^{\circ}$, intensity, noise.
\end{itemize}
\vspace{4pt}
\begin{block}{\small Why it works}
{\small Frozen prior anchors the scarce Severe class; attention zooms into the small lesion.}
\end{block}
\column{.48\textwidth}
\centering
\includegraphics[width=\linewidth]{rsna_class_distribution_en.png}\\[2pt]
{\scriptsize RSNA 2024: Severe stays under 5\% in all three conditions.}
\end{columns}
\end{frame}
...
\begin{itemize}
    ...
    \item[--] Grad-CAM~\cite{selvaraju2017gradcam}: $p(\text{Severe})$ $0.26 \to 0.53$.
\end{itemize}
\column{.5\textwidth}
\centering
...
\centering
\includegraphics[width=.88\textwidth]{ablation_hybrid_vs_naked.png}
\vspace{1pt}
\begin{itemize}\scriptsize
    \item[--] 8 \textbf{unseen} labels, no SPIDER training: only new prompts. Wins on disc morphology (\bst{0.587} vs.\ 0.433), loses on distant labels (all-8: 0.394 vs.\ 0.362).
\end{itemize}
\end{frame}
