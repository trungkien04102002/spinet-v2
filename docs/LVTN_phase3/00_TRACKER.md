...
| **SOTA comparison** | 🟢 **ĐÃ CHẠY XONG** | **Dòng cũ đã lỗi thời.** 2 phương pháp x 3 seed đã chạy: 6 log trong `experiments/sota_comparison/logs/`, metrics JSON trong `checkpoints/`. Số đã kiểm lại khớp bảng trong `paper/bao_cao_phase3`. Hybrid thua ~11 điểm Accuracy nhưng Severe Recall 48.6% so với 26.0% (gần gấp đôi). |
...
**TIẾN ĐỘ CODE (2026-07-20):** #0 XONG (kết quả ở bảng trên). **#1 prep T1 XONG** (`rsna_preprocessed_t1/`, 19,689 crops per-side, 1972/1973 study; `prep_t1_crops.py`+`train_t1_foraminal.py` sẵn) → chỉ còn GPU train foraminal-on-T1. **#2/#3 multi-view model XONG + verify** (`experiments/multiview/`, concat + gated leader-supporter, per-side T1; smoke-test gate foraminal = [T2 0.27, T1 0.73] đúng prior) → chỉ còn GPU train (concat + gated, seed 42). **CÒN GPU:** #1 train, #2/#3 train (mỗi cái 1 seed thử), SOTA (`run_all.sh`). App: SpineNet gốc integration (đang build).
