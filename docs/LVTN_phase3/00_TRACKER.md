...
|---|---|---|---|---|---|
| **0** | **Threshold/calibration** trên checkpoint sẵn: dump logits val 1 lần → coordinate-descent max macro-F1 / match cost 1:2:4 + logit-adjustment + τ-norm | **Không** | Vài giờ | **✅ XONG (CBAM seed42):** +0.039 macro-F1 held-out. Fix CANAL Severe (0.54→0.61) nhưng KHÔNG cứu foraminal Severe → foraminal là vấn đề representation → cần #1. Logit-adj đẩy Severe recall 0.36→0.77 (đổi precision). τ-norm vô nghĩa với Hybrid (cosine). Hybrid dump chờ 4090. | 🟢 |
