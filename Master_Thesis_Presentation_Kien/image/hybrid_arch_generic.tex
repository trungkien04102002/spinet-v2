...
% ---- top branch: volumetric attention branch ----
\foreach \i in {0,1,2,3}{
  \node[cnn] (c\i) at (2.7+\i*0.62,1.95) {};
}
\node[sub] (cbamlab) at (3.63,3.05) {\textbf{Volumetric attention branch}\\(CBAM-3D ResNet-34)};

% ---- bottom branch: multimodal branch ----
\begin{scope}
...
\end{scope}
\node[sub] at (2.8,-2.7) {MRI slices};
\node[vit] (vit) at (4.5,-1.6) {\textbf{Multimodal}\\\textbf{branch}\\\scriptsize(BiomedCLIP\\ViT-B/16)};
\node[proc] (pool) at (6.7,-1.6) {slice\\attention\\pool};

% ---- fusion ----
\node[op] (cat) at (8.7,0.18) {$\oplus$};
\node[sub,left=1mm of cat] {concat\\\scriptsize$1024$};
\node[fuse] (mlp) at (11.35,0.18) {\textbf{Fusion head}\\\scriptsize(MLP $1024{\to}768{\to}512$, L2)};

% ---- cosine head + prompts + outputs ----
\node[head] (cos) at (14.5,0.18) {cosine\\similarity};
\node[txt] (prompt) at (14.5,2.4) {text prompts $\{q_k\}$\\$\to$ multimodal text encoder};
\node[outbox] (rsna) at (16.9,1.5) {Trained label set\\\scriptsize(e.g.\ RSNA severity)};
\node[outbox] (spider) at (16.9,-1.15) {New label set,\\zero-shot\\\scriptsize(e.g.\ SPIDER)};
...
\node[feat,above] at (8.05,-1.6) {$f_{\mathrm{bmc}}{\in}\mathbb{R}^{512}$};
